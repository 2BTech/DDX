...
## Logging
Currently, no actual logging is done.  The only thing that happens is it echoes log messages to stdout/stderr.  Two main types of logging should happen.  First, the Logger class should maintain a list of all the connected devices which are listening to log messages; these should get a copy of each logline.  Also, there should be file logs that save logs to disk.  It should keep track of the size of the logging directory and 
